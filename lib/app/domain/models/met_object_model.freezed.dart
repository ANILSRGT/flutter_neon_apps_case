...
as int?,objectID: freezed == objectID ? _self.objectID : objectID // ignore: cast_nullable_to_non_nullable
...
@override@Unique() final  int? objectID;
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,localId,objectID,isHighlight,accessionNumber,accessionYear,isPublicDomain,primaryImage,primaryImageSmall,const DeepCollectionEquality().hash(_additionalImages),const DeepCollectionEquality().hash(_constituents),department,objectName,title,culture,period,dynasty,reign,portfolio,artistRole,artistPrefix,artistDisplayName,artistDisplayBio,artistSuffix,artistAlphaSort,artistNationality,artistBeginDate,artistEndDate,artistGender,artistWikidata_URL,artistULAN_URL,objectDate,objectBeginDate,objectEndDate,medium,dimensions,const DeepCollectionEquality().hash(_measurements),creditLine,geographyType,city,state,county,country,region,subregion,locale,locus,excavation,river,classification,rightsAndReproduction,linkResource,metadataDate,repository,objectURL,const DeepCollectionEquality().hash(_tags),objectWikidata_URL,isTimelineWork,GalleryNumber]);
